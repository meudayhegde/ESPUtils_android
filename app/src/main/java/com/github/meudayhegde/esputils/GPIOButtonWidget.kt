...
class GPIOButtonWidget: AppWidgetProvider() {
                    ...
                    for(j in 0 until pinJson.length()){
                        ...
                        if(pinNumber == gpioObj.getInt(Strings.espResponsePinNumber)){
                            ...
                            Handler(Looper.getMainLooper()).post {
                                if(pinValue == 1) remoteViews.setImageViewResource(R.id.widget_lamp_icon, R.drawable.icon_lamp_on)
                            }
                        }
                    }
                }else{
                    Handler(Looper.getMainLooper()).post {
                        remoteViews.setImageViewResource(R.id.widget_lamp_icon, R.drawable.icon_lamp_offline)
                        Toast.makeText(context, R.string.message_device_offline, Toast.LENGTH_SHORT).show()
                    }
                }
                        ...
                        Handler(Looper.getMainLooper()).post {
                            if (response.optString(Strings.espResponse) == Strings.espResponseSuccess) {
                                when (response.optInt(Strings.espResponsePinValue)) {
                                    1 -> remoteViews.setImageViewResource(R.id.widget_lamp_icon, R.drawable.icon_lamp_on)
                                    0 -> remoteViews.setImageViewResource(R.id.widget_lamp_icon, R.drawable.icon_lamp)
                                    else -> remoteViews.setImageViewResource(R.id.widget_lamp_icon, R.drawable.icon_lamp_offline)
                                }
                            } else {
                                Toast.makeText(context, R.string.message_device_offline, Toast.LENGTH_SHORT).show()
                                remoteViews.setImageViewResource(R.id.widget_lamp_icon, R.drawable.icon_lamp_offline)
    ...
    override fun onDeleted(context: Context?, appWidgetIds: IntArray?){
        ...
        appWidgetIds?.forEach {
            context?.getSharedPreferences(it.toString(), Context.MODE_PRIVATE)?.edit()?.clear()?.apply()
        }
    }
}