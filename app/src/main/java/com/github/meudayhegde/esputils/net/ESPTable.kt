...
class ESPTable private constructor(context: Context) {
    private val sharedPref =
        context.getSharedPreferences(Strings.sharedPrefNameARPCache, Context.MODE_PRIVATE)
    ...
    fun getIpFromMac(mac: String, listener: ((address: String?) -> Unit)? = null): String? {
        if (listener != null) ThreadHandler.runOnThread(ThreadHandler.ESP_MESSAGE) {
            ...
            for (j in 0 until MAX_RETRY) for (i in 0 until addresses.length()) {
    ...
    private fun startScan(
        macToPropMap: MutableMap<String, DeviceProperties>? = null,
        ...
        ThreadHandler.runOnThread(ThreadHandler.ESP_MESSAGE) {
            ...
            while (true) {
                ...
                val macAddress =
                    JSONObject(String(packet.data).trim { it <= ' ' }).optString(Strings.espResponseMac)
    ...
    private fun sendBroadcastMessage(broadcastAddress: InetAddress, message: String) {
        ...
        val sendPacket =
            DatagramPacket(sendData, sendData.size, broadcastAddress, ESPUtilsApp.UDP_PORT_ESP)
    ...
    companion object {
        ...
        fun getInstance(context: Context? = null): ESPTable {
            return instance ?: ESPTable(
                context
                    ?: throw NullPointerException("${(ESPTable::class.java).simpleName} instance not available")
            )
        }
    }
