...
class ESPUtilsApp : Application() {
    ...
    companion object {
        ...
        private fun getAbsolutePath(
            root: File?,
            vararg dirFileNames: Any = emptyArray(),
            context: Context? = contextRef?.get()
        ): String {
         ...
         */
        fun getPrivateFile(
            vararg dirFileNames: Any = emptyArray(), context: Context? = contextRef?.get()
        ): File {
            return File(
                getAbsolutePath(
                    context?.filesDir ?: Environment.getExternalStorageDirectory(), *dirFileNames
                )
            )
        }
