...
class RemoteButtonWidget : AppWidgetProvider() {
    ...
    override fun onUpdate(
        ...
        for (i in 0 until arr.length()) iconDrawableList[i] = arr.getResourceId(i, 0)
        arr.recycle()
        ...
        for (widgetId in allWidgetIds) {
            ...
            if (buttonProp.icon != 0) remoteViews.setTextViewCompoundDrawables(
                R.id.widget_button, iconDrawableList[buttonProp.icon], 0, 0, 0
            )
    ...
    override fun onReceive(context: Context?, intent: Intent?) {
        ...
        if (intent?.action == Strings.widgetIntentActionClick) {
            ...
            val remoteViews =
                RemoteViews(context?.packageName, R.layout.widget_layout_remote_button)
            ...
            remoteProp.deviceProperties.getIpAddress { address ->
                ...
                if (address == null) {
                    handler.post {
                        Toast.makeText(context, R.string.message_device_offline, Toast.LENGTH_LONG)
                            .show()
                ...
                SocketClient.sendIrCode(address, userName, password, buttonProp.jsonObj) { result ->
                    handler.post {
                        if (result.contains(Strings.espResponseSuccess)) Toast.makeText(
                            context,
                            ...
                            context,
                            context.getString(R.string.message_device_not_connected),
                            Toast.LENGTH_LONG
                        ).show()
                    }
                }
    ...
    private fun getJSONObject(context: Context, widgetID: Int, views: RemoteViews): ArrayList<Any> {
        ...
        if (buttonInfo.isNullOrEmpty()) {
            ...
            if (!queuedButton.isNullOrEmpty()) {
                ...
                Toast.makeText(
                    context, R.string.message_remote_btn_not_configured, Toast.LENGTH_LONG
                ).show()
