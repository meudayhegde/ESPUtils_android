...
class RemoteProperties(
    val remoteConfigFile: File, private val eventListener: EventListener? = null
) {
    ...
    var deviceConfigFileName: String = jsonObj.optString(Strings.remotePropDevPropFileName, "")
        set(value) {
            ...
            update()
            deviceProperties =
                ESPUtilsApp.devicePropList.find { it.deviceConfigFile.name == deviceConfigFileName }
                    ?: DeviceProperties(
                        ESPUtilsApp.getPrivateFile(
                            Strings.nameDirDeviceConfig, deviceConfigFileName
                        )
                    )
        }
    ...
    private fun JSONArray.index(obj: JSONObject): Int {
        for (position in 0 until this.length()) {
            if (getJSONObject(position).optLong(Strings.btnPropBtnId) == obj.getLong(Strings.btnPropBtnId)) return position
        }
        return -1
    }
